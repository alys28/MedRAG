...
if openai.api_key is None:
    ...
    # TODO: The 'openai.api_base' option isn't read in the client API. You will need to pass it when you instantiate the client, e.g. 'OpenAI(base_url=config["api_base"])'
    # client = AzureOpenAI(api_version=config["api_version"], api_key=config["api_key"], base_url=config["api_base"])
...
class MedRAG:
    ...
    def __init__(self, llm_name="OpenAI/gpt-3.5-turbo-16k", rag=True, retriever_name="MedCPT", corpus_name="Textbooks", db_dir="./corpus", cache_dir=None):
        ...
        if self.llm_name.split('/')[0].lower() == "openai":
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(self.llm_name, offload_folder="./model_offload", cache_dir=self.cache_dir)
            ...
            self.model = transformers.pipeline(
                ...
                device_map="auto",
                model_kwargs={"cache_dir":self.cache_dir, "offload_folder":"./model_offload",},
            )
